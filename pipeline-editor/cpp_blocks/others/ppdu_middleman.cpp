...
// ============================================================
// PPDU Middleman  (test/bypass block for PPDU layer)
//
// Sits between ppdu_encapsulate and ppdu_decapsulate for
// isolated testing.  Will be removed once PPDU layer is verified.
//
// Inputs:
//   in[0]: DATA pipe from ppdu_encapsulate          (1515 bytes/pkt)  <- FIRST
...
//
// Outputs:
//   out[0]: lip + DATA -> ppdu_decapsulate           (1519 bytes/pkt)
//             Bytes 0-3: lipBits uint32 LE (from in[1])
//             Bytes 4-1518: DATA (full 1515 bytes)
//   out[1]: SIGNAL -> ppdu_decapsulate               (3 bytes/pkt)
//
// Protocol per batch:
...
//   4. Send SIGNAL batch      (out[1]) -- SIGNAL goes first to decapsulate
...
struct PpduMiddlemanData {
    int frameCount;
};

PpduMiddlemanData init_ppdu_middleman(const BlockConfig& config) {
    PpduMiddlemanData data;
    data.frameCount = 0;

    printf("[PpduMiddleman] Staged I/O mode (CORRECTED)\n");
    printf("[PpduMiddleman] in[0]: DATA (1515 bytes/pkt)\n");
    printf("[PpduMiddleman] in[1]: RATE/LIP (5 bytes/pkt) = [rate_val(1B) | lipBits uint32 LE(4B)]\n");
    printf("[PpduMiddleman] in[2]: Feedback (3 bytes/pkt)\n");
    printf("[PpduMiddleman] out[0] layout: [lipBits(4B LE)|DATA(1515)] = 1519 bytes/pkt\n");
    printf("[PpduMiddleman] out[1] layout: SIGNAL (3 bytes/pkt)\n");
    printf("[PpduMiddleman] Sequence per batch:\n");
    printf("[PpduMiddleman]   1. Read DATA batch    (in[0], 1515 bytes/pkt) -- FIRST\n");
    printf("[PpduMiddleman]   2. Read RATE/LIP      (in[1], 5 bytes/pkt)    -- extract lipBits\n");
    printf("[PpduMiddleman]   3. Send SIGNAL batch  (out[1], 3 bytes/pkt)   -- SIGNAL first\n");
    printf("[PpduMiddleman]   4. Wait for feedback  (in[2], 3 bytes/pkt)    -- gate\n");
    printf("[PpduMiddleman]   5. Send lip+DATA      (out[0], 1519 bytes/pkt)-- DATA second\n");
    printf("[PpduMiddleman] Ready\n");

    return data;
}

void process_ppdu_middleman(
    const char** pipeIn,
    const char** pipeOut,
    PpduMiddlemanData& customData,
    ...
    // ===== STEP 1: Read DATA batch first (rate measurement pipe) =====
    ...
    for (int i = 0; i < actualCount; i++) {
        ...
        if (isFirstBatch && i < DEBUG_PKT_LIMIT) {
            fprintf(stderr, "[PpduMiddleman] DBG pkt[%d] lipBits=0x%08X (%u bits)\n",
    ...
    for (int i = 0; i < actualCount; i++) {
        ...
        if (isFirstBatch && i < DEBUG_PKT_LIMIT) {
            fprintf(stderr, "[PpduMiddleman] DBG pkt[%d] lipBits=0x%08X -> header set\n",
                    i, (unsigned)lipBits);
        }
    }
...
int main(int argc, char* argv[]) {
    if (argc < 6) {
        fprintf(stderr,
            "Usage: ppdu_middleman <pipeInData> <pipeInRate> <pipeInFeedback>"
    ...
    BlockConfig config = {
        ...
        false,                      // ltr (middleman)
        true,                       // startWithAll
        "PPDU middleman: lipBits uint32 LE (4B) + DATA (1515B) = 1519B"
    };

    run_manual_block(pipeIns, pipeOuts, config, process_ppdu_middleman, init_ppdu_middleman);
    return 0;
}