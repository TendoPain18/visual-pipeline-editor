...
//
// Outputs:
//   out[0]: lip + DATA -> ppdu_decapsulate  (1517 bytes/pkt)  <- FIRST
...
PpduMiddlemanData init_ppdu_middleman(const BlockConfig& config) {
    ...
    printf("[PpduMiddleman] Staged I/O mode\n");
    printf("[PpduMiddleman] out[0] layout: [lip_lo(1)|lip_hi(1)|DATA(1515)]\n");
    printf("[PpduMiddleman] Sequence per batch:\n");
    printf("[PpduMiddleman]   1. Read DATA batch    (in[0], 1515 bytes/pkt) -- FIRST\n");
    printf("[PpduMiddleman]   2. Read RATE/LIP      (in[1], 3 bytes/pkt)   -- lip only\n");
    ...
    printf("[PpduMiddleman]   5. Send lip+DATA      (out[0], 1517 bytes/pkt)-- DATA second\n");
...
void process_ppdu_middleman(
    ...
    for (int i = 0; i < actualCount; i++) {
        ...
        if (isFirstBatch && i < DEBUG_PKT_LIMIT) {
            printf("[PpduMiddleman] DBG pkt[%d] lip=%d\n", i, lip);
        }
    }
    ...
    for (int i = 0; i < actualCount; i++) {
        ...
        if (isFirstBatch && i < DEBUG_PKT_LIMIT) {
            printf("[PpduMiddleman] DBG pkt[%d] lip=%d -> header set\n", i, lip);
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    BlockConfig config = {
        ...
        false,                      // ltr (middleman)
        true,                       // startWithAll
        "PPDU test middleman: SIGNAL first, gate on feedback, lip+DATA second"
    };
