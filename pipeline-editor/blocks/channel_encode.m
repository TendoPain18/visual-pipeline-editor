function channel_encode(pipeIn, pipeOut)
% CHANNEL_ENCODE - Channel encoding block with protocol
%
% @BlockConfig
% name: ChannelEncode
% inputs: 1
...
% description: Channel encoder (rate 1/2)
% @EndBlockConfig
    ...
    try
        ...
        while true
            ...
            binaryData = de2bi(double(typecast(data, 'uint8')), 8, 'left-msb');
            ...
            for c = 1:numChunks
                ...
                codedChunk = convenc(double(chunk(:)), trellis);
                coded = [coded; codedChunk];
            end
            
            padLen = mod(8 - mod(length(coded), 8), 8);
            if padLen > 0
                coded = [coded; zeros(padLen, 1)];
            end
            
            codedBytes = bi2de(reshape(coded, 8, []).', 'left-msb');
            outputData = int8(typecast(uint8(codedBytes), 'int8'));
